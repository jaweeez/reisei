...
export async function POST(req: Request) {
  ...
  let b: { honestyAccepted?: unknown; reachOutPreference?: unknown };
  try {
    b = await req.json();
  } catch {
  ...
  const preference = REACH_OUT.has(b.reachOutPreference as ReachOutPreference)
    ...
    : null;
  if (!accepts && !preference) return Response.json({ error: 'Nothing to update.' }, { status: 400 });

  const profile = await withUser(userId, async (c) =>
    (
      await c.query(
        `insert into accountability_profiles (user_id, honesty_acknowledged_at, reach_out_preference, updated_at)
         values (current_app_user(), case when $1 then now() else null end, $2, now())
